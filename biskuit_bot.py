...
def check_client_data(report_text):
    ...
    try:
        ...
        if not (MIN_AGE <= age <= MAX_AGE): errors.append(f"❌ Age must be {MIN_AGE}-{MAX_AGE}. Please Check Sir @DLmktp1\_T389")
    except:
    ...
    try:
        ...
        if hours > MAX_HOURS: errors.append(f"❌ Working hours cannot exceed {MAX_HOURS}h. Please Check Sir @DLmktp1\_T389")
    except:
