...
def check_client_data(report_text):
    ...
    if not link or ('.' not in link):
        ...
        errors.append("❌ Duplicate Error: This link has already been checked. Please Check Sir @DLmktp1_T389")

    # 2. Location
    loc = data.get('Location', '').upper()
    if any(country in loc for country in NOT_DEVELOP_COUNTRIES):
        errors.append("❌ Fails Location rule. Please Check Sir @DLmktp1_T389")

    # 3. Age
    try:
        age_match = re.search(r'\d+', data.get('Age', '0'))
        age = int(age_match.group()) if age_match else 0
        if not (MIN_AGE <= age <= MAX_AGE): errors.append(f"❌ Age must be {MIN_AGE}-{MAX_AGE}. Please Check Sir @DLmktp1_T389")
    except:
    ...
    try:
        ...
        if salary < MIN_SALARY: errors.append(f"❌ Salary must be at least ${MIN_SALARY}. Please Check Sir @DLmktp1_T389")
    except:
    ...
    try:
        ...
        if hours > MAX_HOURS: errors.append(f"❌ Working hours cannot exceed {MAX_HOURS}h. Please Check Sir @DLmktp1_T389")
    except:
